#!/usr/bin/env python
"""Train FusedGravNet on Aohba mixed graph-cache shards with voxel tensors."""
...
def train(args: argparse.Namespace) -> None:
    ...
    model_name = f"{model_tag}_6b_h128_{args.dataset_tag}"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_acc, _ = run_epoch(
            model, train_loader, criterion, optimizer, device, True, args.max_train_batches
        )
        val_loss, val_acc, _ = run_epoch(
            model, val_loader, criterion, optimizer, device, False, args.max_val_batches
        )
        scheduler.step()
