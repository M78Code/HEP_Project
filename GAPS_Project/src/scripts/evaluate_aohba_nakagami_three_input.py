#!/usr/bin/env python
"""Evaluate Nakagami-style three-input CNN+DNN on exported Aohba arrays."""
...
from GAPS_Project.src.scripts.train_aohba_nakagami_three_input_amp import (
    NakagamiStyle3InputDataset,
)
...
def infer(model, loader, device, total_batches, amp_enabled: bool, amp_dtype: torch.dtype):
    model.eval()
    labels, scores = [], []
    for voxel, tof_paddle, tof_feature, label in tqdm(loader, total=total_batches, desc="test", dynamic_ncols=True):
...
def main() -> None:
    ...
    dataset = NakagamiStyle3InputDataset(args.data_dir, "test")
    ...
    betas = np.load(args.data_dir / "test_nakagami_style_4M" / "betas.npy")
    ...
    plt.plot(
        ...
        label=f"Nakagami-style 3-input (AUC={metrics['auc']:.4f})",
    )
    plt.yscale("log")
    plt.xlim(0.5, 1.0)
