...
GNN runs on Nakagami fixed-grid CSV data.  It uses only:
  ...
  - voxels.npy      : Si(Li) voxel map, (N, 10, 12, 12)
  - tof_primary.npy : 11-D auxiliary/TOF feature vector
  - labels.npy      : binary label

It deliberately ignores tof_paddles.npy and beta so that the input matches the
two-branch CNN+DNN architecture shown in Nakagami thesis Fig. A.6 / Sec. 7.1.1.
"""
...
def save_evaluation(
    ...
    metrics = {
        ...
        "input": {
            "voxel": "voxels.npy",
            "tof": "tof_primary.npy",
            "ignored": ["tof_paddles.npy", "betas.npy"],
        },
    }
...
def main() -> None:
    ...
    print("input      : voxels.npy + tof_primary.npy (tof_paddles/beta ignored)", flush=True)
